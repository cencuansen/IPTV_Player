  ...
  <div v-if="open" class="modal-overlay" @click="onOverlayClick">
    <div class="modal modal-lg">
      ...
      <div class="modal-body">
        ...
        <div v-if="sourcesStore.historyStatus.text" class="history-status" :class="sourcesStore.historyStatus.kind">
        ...
        <div class="history-list">
          ...
          <div v-for="src in sourcesStore.sources" :key="src.id" class="history-item"
            :class="{ current: src.id === sourcesStore.currentSourceId }">
            ...
            <div class="history-actions">
              <button class="btn btn-sm btn-primary" :disabled="sourcesStore.historyBusy" title="直接显示本地保存的数据；本地无数据时先获取"
                @click="apply(src.id)">应用</button>
              <button class="btn btn-sm btn-primary" :disabled="sourcesStore.historyBusy" title="重新获取数据并更新本地后应用"
                @click="update(src.id)">更新</button>
              <button class="btn btn-sm btn-primary" :disabled="sourcesStore.historyBusy" title="查看 m3u 路径/地址与文件内容"
                @click="emit('open-detail', src.id)">详情</button>
              <button class="btn btn-sm btn-danger" :disabled="sourcesStore.historyBusy" title="删除该导入记录"
                @click="remove(src.id)">删除</button>
            </div>
          </div>
        </div>
      </div>
